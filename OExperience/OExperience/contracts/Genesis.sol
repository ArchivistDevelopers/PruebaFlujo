...
contract OEGenesis is ERC721Enumerable, ERC2981, Ownable {
    ...
    /*
     * {OEGenesis} - Returns OEToken address
     */
    function getTokenContract() public view returns(address) {
        return address(OEToken); 
    }
    ...
    function teamMint(uint8 numToMint) external onlyOwner {
        ...
        require(
            numToMint > 0,
            "OE GENESIS: number of tokens must be higher than zero"
        );
        require(
            Id + numToMint <= MAX_SUPPLY,
            "OE GENESIS: max supply reached"
        );
    ...
    function getKeyLevel(uint256 tokenId) public view returns(uint256) {
        require(
            _exists(tokenId),
            "OE GENESIS: query for non-existent token"
        );
